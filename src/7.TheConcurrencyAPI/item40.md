...
<u>这两个问题——不保证操作的原子性以及对代码重排顺序没有足够限制——解释了为什么`volatile`在多线程编程中没用</u>，但是没有解释它应该用在哪。简而言之，<u>它是用来告诉编译器，它们处理的内存有不正常的表现。</u>
...
<u>`volatile`是告诉编译器我们正在处理特殊内存。意味着告诉编译器“不要对这块内存执行任何优化”。所以如果`x`对应于特殊内存，应该声明为`volatile`：</u>
